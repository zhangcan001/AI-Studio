    ...
    <details className="prompt-library-panel">
      ...
      <div className="prompt-library-layout">
        <div className="prompt-library-list" aria-label="提示词库条目">
          ...
          {entries.map((entry) => <button type="button" key={entry.id} className={entry.id === selectedId ? "prompt-library-list-item active" : "prompt-library-list-item"} onClick={() => setSelectedId(entry.id)}><strong>{entry.name}</strong><span>{entry.kind === "prompt" ? "提示词" : "片段"} · {entry.versionCount} 个版本</span><small>{entry.tags.join(" · ") || "无标签"}</small></button>)}
        </div>
        <div className="prompt-library-detail">
          ...
          {detail && (
            <>
              <div className="prompt-library-detail-heading"><div><strong>{detail.name}</strong><span>{detail.kind === "prompt" ? "提示词" : "片段"} · {detail.versionCount} 个版本</span></div><button type="button" className="quiet-button" onClick={() => void removeEntry()} disabled={saving}>删除</button></div>
