...
function promptPreview(text: string): string {
  ...
  if (normalized.length <= 96) return normalized || "未配置 Prompt";
...
export function ShotBulkConfigPanel({
  ...
  function applyPreset(preset: BulkSelectionPreset) {
    ...
    reportNotice(
      preset === "all"
        ? `已选择全部 ${shots.length} 个 Shot。`
        : preset === "ready"
          ? `已选择当前${stageLabel(stage)} Ready 的 Shot。`
          : `已选择当前${stageLabel(stage)}未配置的 Shot。`,
    );
  }
  ...
  async function configureStage(nextStage: ShotStage) {
    ...
    await runAction(
      ...
      `已提交 ${selectedCount} 个 Shot 的${stageLabel(nextStage)}批量配置。`,
    );
  }

  async function assignPrompt() {
    if (!onBulkPrompt) {
      reportNotice("已保留批量 Prompt 入口；当前组件不会绕过 ShotService 直接写数据库。 ");
      return;
    }
    if (!promptText.trim()) {
      reportError("请输入要批量应用的 Prompt。清除 provenance 请使用后端批量命令。" );
      return;
    }
    await runAction(
      "prompt",
      (shotIds) => onBulkPrompt(stage, shotIds, promptText),
      `已提交 ${selectedCount} 个 Shot 的批量 Prompt 操作；保存后仍按快照语义生效。`,
    );
  }
      ...
      async (shotIds) => {
        ...
        if (blocked.length || shotIds.length > plan.maxItems) {
          const details = blocked
            .map((row) => row ? `${row.name}：${row.blockingReasons.join("；")}` : "所选 Shot 不在当前项目批次计划中")
            .join("；");
        ...
        try {
          ...
          reportNotice(`${stageLabel(nextStage)} Batch 已创建并开始严格串行执行；结果仍需人工 Review/Select。`);
        } catch (startError: unknown) {
          reportNotice(`${stageLabel(nextStage)} Batch 已创建为待启动状态：${batch.name}。${toUserMessage(startError)}`);
        }
      },
      `${stageLabel(nextStage)} Batch 操作已完成；生成结果仍需人工 Review/Select。`,
    );
  }

  return (
    <section className="shot-bulk-config-panel" aria-label="批量 Shot 配置">
      <div className="pipeline-section-heading">
        <div>
          <span className="section-label">批量配置</span>
          <h3>Shot 批量生产入口</h3>
          <p className="pipeline-muted">先选择 Shot，再进入图片/视频配置、Prompt 或现有 ShotBatchService 队列。</p>
        </div>
        <span className="pipeline-selection-count">{selectedCount} 个 Shot 已选择</span>
      </div>
      ...
      <div className="pipeline-selection-toolbar" aria-label="Shot 选择操作">
        ...
        <button type="button" className="quiet-button" onClick={() => applyPreset("ready")} disabled={Boolean(busyAction)}>选择 Ready</button>
      ...
      <div className="pipeline-action-grid">
        ...
        <button type="button" onClick={() => void createBatch("image")} disabled={Boolean(busyAction) || !selectedCount}>
          {busyAction === "batch-image" ? "正在创建图片 Batch…" : "创建图片 Batch"}
        </button>
        <button type="button" onClick={() => void createBatch("video")} disabled={Boolean(busyAction) || !selectedCount}>
          {busyAction === "batch-video" ? "正在创建视频 Batch…" : "创建视频 Batch"}
        </button>
      </div>

      <label className="pipeline-prompt-editor">
        <span>批量应用当前阶段 Prompt</span>
        <textarea value={promptText} onChange={(event) => setPromptText(event.target.value)} rows={3} placeholder="输入后点击“批量 Prompt”；文本会以当前阶段快照写入所选 Shot。" disabled={Boolean(busyAction)} />
      </label>

      <div className="pipeline-shot-table-wrap">
        <table className="pipeline-shot-table">
          <thead>
            <tr><th aria-label="选择" /><th>Shot</th><th>当前 Prompt 快照</th><th>图片状态</th><th>视频状态</th></tr>
          </thead>
          <tbody>
            {shots.map((shot) => (
              <tr key={shot.id} className={selectedIds.has(shot.id) ? "pipeline-shot-row-selected" : ""}>
                ...
                <td><span className="pipeline-prompt-preview">{promptPreview(shot.stagePrompts?.find((prompt) => prompt.stage === stage)?.promptText ?? shot.promptText)}</span><small>{shot.stagePrompts?.find((prompt) => prompt.stage === stage)?.promptEntryId && shot.stagePrompts?.find((prompt) => prompt.stage === stage)?.promptVersionId ? `Library version ${shot.stagePrompts?.find((prompt) => prompt.stage === stage)?.promptVersionId?.slice(-8)}` : "手工/导入阶段快照"}</small></td>
            ...
            {!shots.length && <tr><td colSpan={5}><p className="empty-state">当前项目还没有 Shot。</p></td></tr>}
          </tbody>
        </table>
      </div>

      <p className="pipeline-human-review-note"><strong>人工 Review 保持不变：</strong>批量操作不会自动选择第一张图片、最新图片或第一个视频；图片生成后停在 IMAGE_REVIEW，视频生成后停在 VIDEO_REVIEW。</p>
