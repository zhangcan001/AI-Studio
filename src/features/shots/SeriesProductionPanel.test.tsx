...
const plan: SeriesProductionPlan = {
  ...
  episodes: [
    ...
    { episodeId: "episode-2", episodeName: "夜战", episodeOrdinal: 1, sceneTotal: 1, shotTotal: 1, done: 0, prepared: 0, eligible: 1, blocked: 1, classification: "PARTIAL", canPrepare: true, existingBatchIds: [], blockingReasons: ["缺少图片提示词"] },
  ],
};
...
describe("SeriesProductionPanel", () => {
  it("renders the series/stage selectors, summary, episode table, strict default and safety copy", () => {
    ...
    expect(html).toContain("系列选择");
    expect(html).toContain("图片");
    expect(html).toContain("第一季");
    expect(html).toContain("可生产集");
  ...
  it("keeps strict mode, confirmation, busy lock and preservation copy explicit", () => {
    ...
    expect(seriesPrepareConfirmation("第一季", 4, 27, "image")).toContain("预计最多创建 27 个待启动批次");
