...
export function ShotBatchReviewBoard({ projectId, shots, assets, stage, busy, onAssetsLoaded, onSelect, onRetry, onOpenTask, onOpenShot, onOpenAsset, reviewBatchId, initialReviewItemId, reviewBatchLoader = getProductionBatchReviewProductivity, onOpenProductionQueue }: Props) {
  ...
  async function confirmAndApprove(candidate: ReviewCompareCandidate, item: ReviewCompareItem) {
    ...
    try { await setProductionReviewStatus({ projectId, batchId: reviewBatchId, itemId: item.id, status: "APPROVED" }); setNotice("已选择结果并通过审核；下一步可继续审片，或查看对应 Shot / Asset。此动作不代表发布。"); setSelectionContinuation({ shotId: item.shotId, assetId: candidate.id }); await reloadReview(); }
  ...
  async function createReworkBatch(item: ReviewCompareItem) {
    ...
    try {
      ...
      setNotice(`返工批次已创建（${result.selectedCount} 项），状态为待启动，尚未开始真实生产。请打开生产队列后明确点击“开始生产”。`);
  ...
  if (detailedCompareAvailable) {
    ...
    return (
      <section className="shot-batch-review-board" aria-label={`${stageLabel(stage)}批量复核`}>
        <div className="shot-block-heading"><div><span className="section-label">人工复核</span><h3>{stage === "image" ? "关键帧候选复核" : "视频候选复核"}</h3></div><span className="shot-inline-note">A/B 仅用于比较；只有显式选择结果并通过审核才会写入 Shot。此动作不代表发布。</span></div>
        <div className="shot-batch-review-filters" role="toolbar" aria-label="批量复核筛选">
          {([ ["ALL", `全部 ${mappedItems.length}`], ["UNREVIEWED", `待审核 ${counts.unreviewed}`], ["APPROVED", `已通过 ${counts.approved}`], ["STARRED", `已标星 ${counts.starred}`], ["REGENERATE", `待返工 ${counts.regenerate}`], ["REJECTED", `已拒绝 ${counts.rejected}`], ["FAILED", `失败 ${counts.failed}`] ] as const).map(([value, label]) => <button key={value} type="button" className={filter === value ? "review-filter-active" : "quiet-button"} aria-pressed={filter === value} onClick={() => setFilter(value)}>{label}</button>)}
        </div>
...
function LegacyReviewBoard({ projectId, shots, assets, stage, busy, onAssetsLoaded, onSelect, onRetry, onOpenTask, onOpenLocalCompare }: Omit<Props, "reviewBatchId" | "reviewBatchLoader" | "onOpenProductionQueue"> & { onOpenLocalCompare: (shotId: string) => void }) {
  void onAssetsLoaded;
  return <section className="shot-batch-review-board" aria-label={`${stageLabel(stage)}批量复核`}><div className="shot-block-heading"><div><span className="section-label">人工复核</span><h3>{stage === "image" ? "关键帧候选复核" : "视频候选复核"}</h3></div><span className="shot-inline-note">每个镜头必须明确选择结果后才进入下一阶段；选择不代表发布。</span></div><div className="shot-batch-review-grid">{shots.map((shot) => { const status = deriveStageStatus(shot, stage); const links = shot.generationLinks.filter((link) => link.stage === stage); const ids = new Set(links.flatMap((link) => link.task?.outputAssetIds ?? [])); const candidates = assets.filter((asset) => ids.has(asset.id) && (stage === "image" ? asset.assetType === "image" : asset.assetType === "video")); const selectedId = stage === "image" ? shot.selectedImageAssetId : shot.selectedVideoAssetId; const failure = recentShotFailure(shot, stage); return <article key={shot.id} className="shot-batch-review-card"><div className="shot-batch-review-card-heading"><div><strong>{String(shot.ordinal + 1).padStart(2, "0")} · {shot.name}</strong><small>{shotStatusLabels[status]}</small></div>{selectedId && <span className="shot-batch-selected-badge">已选择</span>}</div>{candidates.length > 0 ? <div className="shot-batch-review-candidates">{candidates.map((asset) => <div key={asset.id} className={`shot-batch-review-candidate${selectedId === asset.id ? " selected" : ""}`}><ReviewAssetMedia projectId={projectId} asset={asset} stage={stage} /><div><strong>{asset.name}</strong><small>{selectedId === asset.id ? "当前选择" : "候选结果"}</small></div><button type="button" disabled={busy || selectedId === asset.id} onClick={() => onSelect(shot.id, stage, asset.id, true)}>{selectedId === asset.id ? "已选择" : stage === "image" ? "选择关键帧结果" : "选择视频结果"}</button></div>)}<button type="button" className="quiet-button" disabled={busy} onClick={() => onOpenLocalCompare(shot.id)}>打开 A/B 对比</button></div> : <p className="empty-state">{status === "GENERATING_IMAGE" || status === "GENERATING_VIDEO" ? "任务运行中，等待结果…" : "暂无候选结果"}</p>}{failure && <div className="shot-batch-review-failure"><strong>最近一次任务失败</strong><span>{failure.error?.message ?? "生成任务失败，需要处理"}</span><div><button type="button" className="quiet-button" disabled={busy} onClick={() => onRetry(shot.id, stage)}>重新加入队列</button>{onOpenTask && <button type="button" className="quiet-button" disabled={busy} onClick={() => failure.id && onOpenTask(failure.id)}>查看任务详情</button>}</div></div>}</article>; })}{!shots.length && <p className="empty-state">还没有镜头。</p>}</div></section>;
}
