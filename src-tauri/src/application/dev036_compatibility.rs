...
#[tokio::test]
async fn dev036_backup_v12_preserves_template_source_and_frozen_stage_snapshot() {
    ...
    let archive_path = directory.path().join("dev036-v12.zip");
    service
        .export("dev036-backup", archive_path.clone())
        .await
        .expect("v12 backup should export");
    let inspection = service
        .inspect(archive_path.clone())
        .await
        .expect("v12 backup should inspect");
