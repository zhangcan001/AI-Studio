use crate::application::ports::{
    ...
    ProjectBackupRestorePlan, ProjectBackupRestoreResult, ProjectBackupSnapshot, ProjectRecord,
    RepositoryError,
};
...
impl ProjectBackupRepository for SqliteProjectBackupRepository {
    async fn load_export_snapshot(
        ...
        .await?;
        let asset_versions =
            query_backup_asset_versions(&mut transaction, project_id, &included_asset_ids).await?;
        let asset_relations =
            query_backup_asset_relations(&mut transaction, project_id, &included_asset_ids).await?;
        let generation_tool_usages =
            query_backup_generation_tool_usages(&mut transaction, &included_task_ids).await?;
        let generation_asset_versions = query_backup_generation_asset_versions(
            &mut transaction,
            &included_task_ids,
            &asset_versions,
        )
    ...
    async fn restore_atomic(
        ...
        let result = restore_rows_in_transaction(
            ...
            &plan.asset_version_ids,
            &plan.asset_relation_ids,
            &plan.generation_tool_usage_ids,
            &plan.generation_asset_version_ids,
...
async fn query_prompt_versions(
    ...
    let rows = sqlx::query_as::<_, DbPromptVersion>(
        "SELECT v.id, v.prompt_id, v.version, v.text, v.model_version_id, v.created_at
...
async fn restore_rows_in_transaction(
    ...
    asset_version_ids: &HashMap<String, String>,
    asset_relation_ids: &HashMap<String, String>,
    generation_tool_usage_ids: &HashMap<String, String>,
    ...
    restore_asset_versions_and_relations(
        transaction,
        project,
        document,
        asset_ids,
...
async fn query_backup_asset_versions(
    transaction: &mut Transaction<'_, Sqlite>,
    project_id: &str,
    included_asset_ids: &HashSet<&str>,
) -> Result<Vec<BackupAssetVersion>, RepositoryError> {
    let rows = sqlx::query_as::<_, DbBackupAssetVersion>(
        "SELECT id, project_id, asset_id, version_number, metadata_snapshot, location, checksum, created_at
         FROM asset_versions WHERE project_id = ? ORDER BY asset_id, version_number, id",
    )
    .bind(project_id)
    .fetch_all(&mut **transaction)
    .await
    .map_err(|error| RepositoryError::database(error.to_string()))?;
    rows.into_iter()
        .filter(|row| included_asset_ids.contains(row.asset_id.as_str()))
        .map(|row| {
            Ok(BackupAssetVersion {
                id: row.id,
                project_id: row.project_id,
                asset_id: row.asset_id,
...
async fn query_backup_asset_relations(
    transaction: &mut Transaction<'_, Sqlite>,
    project_id: &str,
    included_asset_ids: &HashSet<&str>,
) -> Result<Vec<BackupAssetRelation>, RepositoryError> {
    let rows = sqlx::query_as::<_, DbBackupAssetRelation>(
        "SELECT id, project_id, source_asset_id, target_asset_id, relation_type, created_at
         FROM asset_relations WHERE project_id = ?
         ORDER BY created_at, id",
    )
    .bind(project_id)
    .fetch_all(&mut **transaction)
    ...
    Ok(rows
        .into_iter()
        .filter(|row| {
            included_asset_ids.contains(row.source_asset_id.as_str())
                && included_asset_ids.contains(row.target_asset_id.as_str())
        })
        .map(|row| BackupAssetRelation {
            id: row.id,
            project_id: row.project_id,
            source_asset_id: row.source_asset_id,
...
async fn query_backup_generation_tool_usages(
    transaction: &mut Transaction<'_, Sqlite>,
    included_task_ids: &HashSet<String>,
) -> Result<Vec<BackupGenerationToolUsage>, RepositoryError> {
    let mut result = Vec::new();
    for task_id in included_task_ids {
        let rows = sqlx::query_as::<_, DbBackupGenerationToolUsage>(
            "SELECT id, generation_id, tool_instance_id, tool_version_id, metadata_json, created_at
             FROM generation_tool_usages WHERE generation_id = ?
             ORDER BY created_at, id",
        )
        .bind(task_id)
        .fetch_all(&mut **transaction)
        ...
        .map_err(|error| RepositoryError::database(error.to_string()))?;
        for row in rows {
            result.push(BackupGenerationToolUsage {
                id: row.id,
...
async fn query_backup_generation_asset_versions(
    transaction: &mut Transaction<'_, Sqlite>,
    included_task_ids: &HashSet<String>,
    asset_versions: &[BackupAssetVersion],
) -> Result<Vec<BackupGenerationAssetVersion>, RepositoryError> {
    let included_version_ids = asset_versions
        .iter()
        .map(|version| version.id.as_str())
        .collect::<HashSet<_>>();
    let mut result = Vec::new();
    for task_id in included_task_ids {
        let rows = sqlx::query_as::<_, DbBackupGenerationAssetVersion>(
            "SELECT id, generation_id, output_id, ordinal, asset_version_id, relation_type, created_at
             FROM generation_asset_versions WHERE generation_id = ?
             ORDER BY created_at, id",
        )
        .bind(task_id)
        .fetch_all(&mut **transaction)
        .await
        .map_err(|error| RepositoryError::database(error.to_string()))?;
        result.extend(rows.into_iter().filter_map(|row| {
            if !included_version_ids.contains(row.asset_version_id.as_str()) {
                return None;
            }
            Some(BackupGenerationAssetVersion {
                id: row.id,
                generation_id: row.generation_id,
                output_id: row.output_id,
                ordinal: row.ordinal,
...
async fn query_backup_referenced_models(
    transaction: &mut Transaction<'_, Sqlite>,
    prompt_versions: &[BackupPromptVersion],
    snapshots: &[BackupSnapshot],
) -> Result<(Vec<BackupModel>, Vec<BackupModelVersion>), RepositoryError> {
    let mut version_ids = HashSet::new();
    for version in prompt_versions {
        if let Some(id) = &version.model_version_id {
            version_ids.insert(id.clone());
        }
    }
    for snapshot in snapshots {
        if let Some(id) = &snapshot.model_version_id {
            version_ids.insert(id.clone());
        }
    }
    ...
    for version_id in &version_ids {
        let row = sqlx::query_as::<_, DbBackupModelVersion>(
        ...
        if let Some(row) = row {
            ...
            model_versions.push(BackupModelVersion {
                id: row.id,
                model_id: row.model_id,
                version: row.version,
                capabilities_json: parse_value(Some(&row.capabilities_json), "model capabilities")?,
                parameter_schema_json: parse_value(
                    Some(&row.parameter_schema_json),
    ...
    for model_id in &model_ids {
        let row = sqlx::query_as::<_, DbBackupModel>(
            "SELECT id, name, provider, type, description, metadata_json, created_at
        ...
        .map_err(|error| RepositoryError::database(error.to_string()))?;
        if let Some(row) = row {
            models.push(BackupModel {
                id: row.id,
                name: row.name,
                provider: row.provider,
                model_type: row.model_type,
                description: row.description,
                metadata_json: parse_value(Some(&row.metadata_json), "model metadata")?,
                created_at: row.created_at,
            });
        }
    }
    models.sort_by(|left, right| left.id.cmp(&right.id));
    model_versions.sort_by(|left, right| left.id.cmp(&right.id));
    Ok((models, model_versions))
}

async fn query_backup_referenced_tools(
    transaction: &mut Transaction<'_, Sqlite>,
    usages: &[BackupGenerationToolUsage],
) -> Result<
    (
        Vec<BackupTool>,
        Vec<BackupToolVersion>,
        Vec<BackupToolCapability>,
        Vec<BackupToolInstance>,
    ),
    RepositoryError,
> {
    ...
    let mut version_ids = HashSet::new();
    for usage in usages {
        instance_ids.insert(usage.tool_instance_id.clone());
        if let Some(id) = &usage.tool_version_id {
            version_ids.insert(id.clone());
        }
    }
    if instance_ids.is_empty() && version_ids.is_empty() {
        return Ok((Vec::new(), Vec::new(), Vec::new(), Vec::new()));
    }

    let mut tool_instances = Vec::new();
    let mut tool_ids = HashSet::new();
    for instance_id in &instance_ids {
        let row = sqlx::query_as::<_, DbBackupToolInstance>(
        ...
        .map_err(|error| RepositoryError::database(error.to_string()))?;
        if let Some(row) = row {
            tool_ids.insert(row.tool_id.clone());
            tool_instances.push(BackupToolInstance {
                id: row.id,
                tool_id: row.tool_id,
                path: row.path,
    ...
    for version_id in &version_ids {
        let row = sqlx::query_as::<_, DbBackupToolVersion>(
            "SELECT id, tool_id, version, observed_at, metadata_json FROM tool_versions WHERE id = ?",
        )
        .bind(version_id)
        .fetch_optional(&mut **transaction)
        .await
        ...
        if let Some(row) = row {
            tool_ids.insert(row.tool_id.clone());
            tool_versions.push(BackupToolVersion {
                id: row.id,
                tool_id: row.tool_id,
                version: row.version,
                observed_at: row.observed_at,
                metadata_json: parse_value(Some(&row.metadata_json), "tool version metadata")?,
            });
        }
    }
    ...
    for tool_id in &tool_ids {
        let row = sqlx::query_as::<_, DbBackupTool>(
            "SELECT id, name, type, description, metadata_json, created_at
             FROM tools WHERE id = ?",
        )
        .bind(tool_id)
        .fetch_optional(&mut **transaction)
        ...
        if let Some(row) = row {
            tools.push(BackupTool {
                ...
                description: row.description,
                metadata_json: parse_value(Some(&row.metadata_json), "tool metadata")?,
                created_at: row.created_at,
            });
        }
        let caps = sqlx::query_as::<_, DbBackupToolCapability>(
            "SELECT tool_id, capability_name, metadata_json FROM tool_capabilities WHERE tool_id = ?
        ...
        .map_err(|error| RepositoryError::database(error.to_string()))?;
        for cap in caps {
            tool_capabilities.push(BackupToolCapability {
                tool_id: cap.tool_id,
                capability_name: cap.capability_name,
                metadata_json: parse_value(Some(&cap.metadata_json), "tool capability metadata")?,
            });
        }
    }
    tools.sort_by(|left, right| left.id.cmp(&right.id));
    tool_versions.sort_by(|left, right| left.id.cmp(&right.id));
    tool_instances.sort_by(|left, right| left.id.cmp(&right.id));
    Ok((tools, tool_versions, tool_capabilities, tool_instances))
}

#[derive(FromRow)]
struct DbBackupAssetRelation {
    id: String,
    project_id: String,
...
#[derive(FromRow)]
struct DbBackupGenerationAssetVersion {
    id: String,
...
#[derive(FromRow)]
struct DbBackupGenerationToolUsage {
    id: String,
    generation_id: String,
...
#[derive(FromRow)]
struct DbBackupToolCapability {
    tool_id: String,
    capability_name: String,
    metadata_json: String,
}

async fn restore_model_registry(
    transaction: &mut Transaction<'_, Sqlite>,
    document: &BackupDocument,
    ...
    for model in &document.models {
        let existing_by_id = sqlx::query_as::<_, (String, String, String)>(
            "SELECT provider, name, type FROM models WHERE id = ?",
        )
        .bind(&model.id)
        .fetch_optional(&mut **transaction)
        .await
        .map_err(|error| RepositoryError::database(error.to_string()))?;
        if let Some((provider, name, model_type)) = existing_by_id {
            if provider == model.provider && name == model.name && model_type == model.model_type {
                model_id_map.insert(model.id.clone(), model.id.clone());
                continue;
            }
            unresolved_models.insert(model.id.clone());
            continue;
        }
        ...
        .await
        .map_err(|error| RepositoryError::database(error.to_string()))?;
        if let Some(existing_id) = existing_by_unique {
            model_id_map.insert(model.id.clone(), existing_id);
            continue;
        }
        ...
        .bind(model.metadata_json.to_string())
    ...
    let mut model_version_map = HashMap::new();
    let mut unresolved_versions = Vec::new();
    for version in &document.model_versions {
        let Some(resolved_model_id) = model_id_map.get(&version.model_id).cloned() else {
            unresolved_versions.push(version.id.clone());
            continue;
        };
        if unresolved_models.contains(&version.model_id) {
            unresolved_versions.push(version.id.clone());
            continue;
        }

        let existing_by_id = sqlx::query_as::<_, (String, String)>(
            "SELECT model_id, version FROM model_versions WHERE id = ?",
        )
        .bind(&version.id)
        .fetch_optional(&mut **transaction)
        .await
        .map_err(|error| RepositoryError::database(error.to_string()))?;
        if let Some((model_id, ver)) = existing_by_id {
            if model_id == resolved_model_id && ver == version.version {
                model_version_map.insert(version.id.clone(), version.id.clone());
                continue;
            }
            unresolved_versions.push(version.id.clone());
            continue;
        ...
        .map_err(|error| RepositoryError::database(error.to_string()))?;
        if let Some(existing_id) = existing_by_unique {
            model_version_map.insert(version.id.clone(), existing_id);
            continue;
        }
        ...
        .bind(&resolved_model_id)
        .bind(&version.version)
        .bind(version.capabilities_json.to_string())
        .bind(version.parameter_schema_json.to_string())
        .bind(&version.created_at)
        .execute(&mut **transaction)
        .await
        .map_err(|error| RepositoryError::database(error.to_string()))?;
    ...
    Ok((model_version_map, unresolved_versions))
}
...
> {
    ...
    for tool in &document.tools {
        ...
        if let Some((name, tool_type)) = existing {
            ...
            unresolved_tools.insert(tool.id.clone());
            continue;
        }
        sqlx::query(
        ...
        .bind(tool.metadata_json.to_string())
    ...
    for capability in &document.tool_capabilities {
        let Some(tool_id) = tool_id_map.get(&capability.tool_id) else {
            continue;
        };
        if unresolved_tools.contains(&capability.tool_id) {
            continue;
        }
        let exists = sqlx::query_scalar::<_, i64>(
            "SELECT COUNT(*) FROM tool_capabilities WHERE tool_id = ? AND capability_name = ?",
        )
        .bind(tool_id)
        ...
        .bind(tool_id)
        .bind(&capability.capability_name)
        .bind(capability.metadata_json.to_string())
        .execute(&mut **transaction)
        .await
        .map_err(|error| RepositoryError::database(error.to_string()))?;
    }
    ...
    for version in &document.tool_versions {
        ...
        let existing_by_id = sqlx::query_as::<_, (String, String)>(
            "SELECT tool_id, version FROM tool_versions WHERE id = ?",
        )
        .bind(&version.id)
        .fetch_optional(&mut **transaction)
        .await
        .map_err(|error| RepositoryError::database(error.to_string()))?;
        if let Some((existing_tool_id, ver)) = existing_by_id {
            if existing_tool_id == tool_id && ver == version.version {
        ...
        .map_err(|error| RepositoryError::database(error.to_string()))?;
        if let Some(existing_id) = existing_by_unique {
            tool_version_map.insert(version.id.clone(), existing_id);
            continue;
        }
        sqlx::query(
        ...
        .bind(&tool_id)
        .bind(&version.version)
        .bind(&version.observed_at)
        .bind(version.metadata_json.to_string())
        .execute(&mut **transaction)
        .await
        .map_err(|error| RepositoryError::database(error.to_string()))?;
        tool_version_map.insert(version.id.clone(), version.id.clone());
    ...
    let mut tool_instance_map = HashMap::new();
    let mut unresolved_instances = Vec::new();
    for instance in &document.tool_instances {
        let Some(tool_id) = tool_id_map.get(&instance.tool_id).cloned() else {
            unresolved_instances.push(instance.id.clone());
            continue;
        };
        if unresolved_tools.contains(&instance.tool_id) {
            unresolved_instances.push(instance.id.clone());
            continue;
        }

        let existing = sqlx::query_as::<_, (String, Option<String>, Option<String>)>(
            "SELECT tool_id, path, endpoint FROM tool_instances WHERE id = ?",
        )
        .bind(&instance.id)
        .fetch_optional(&mut **transaction)
        .await
        .map_err(|error| RepositoryError::database(error.to_string()))?;
        if let Some((existing_tool_id, path, endpoint)) = existing {
            if existing_tool_id == tool_id && path == instance.path && endpoint == instance.endpoint
            {
                // Keep existing row; do not overwrite health. Provenance can reuse the id.
                tool_instance_map.insert(instance.id.clone(), instance.id.clone());
                continue;
            }
            unresolved_instances.push(instance.id.clone());
            continue;
...
async fn restore_asset_versions_and_relations(
    ...
    let storage_by_old = restored_assets
        .iter()
        .map(|asset| (asset.old_id.as_str(), asset.storage_path.as_str()))
        .collect::<HashMap<_, _>>();

    for version in &document.asset_versions {
        let version_id = asset_version_ids.get(&version.id).ok_or_else(|| {
            RepositoryError::integrity(format!("资产版本 ID 映射缺失：{}", version.id))
        })?;
        let asset_id = asset_ids.get(&version.asset_id).ok_or_else(|| {
            RepositoryError::integrity(format!("资产版本缺少资产映射：{}", version.asset_id))
        })?;
        // Always rewrite location onto the restored media path under the NEW
        // project root. Never reuse archived absolute host paths from the package.
        let location = storage_by_old
            .get(version.asset_id.as_str())
            .copied()
            .ok_or_else(|| {
                RepositoryError::integrity(format!(
                    "资产版本缺少恢复后的媒体路径：{}",
        ...
        .bind(version.metadata_snapshot.to_string())
        .bind(location)
...
async fn restore_provenance_lineage(
    transaction: &mut Transaction<'_, Sqlite>,
    document: &BackupDocument,
    task_ids: &HashMap<String, String>,
    asset_version_ids: &HashMap<String, String>,
    ...
    for usage in &document.generation_tool_usages {
        ...
        let tool_version_id = usage
            .tool_version_id
            .as_ref()
            .and_then(|id| tool_version_map.get(id).cloned());
        sqlx::query(
        ...
        .bind(usage.metadata_json.to_string())
