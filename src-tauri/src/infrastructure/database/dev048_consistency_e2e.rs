...
async fn remove_024_for_upgrade_fixture(pool: &sqlx::SqlitePool) {
    for statement in [
        "DROP INDEX IF EXISTS idx_generation_snapshots_prompt_version",
        "ALTER TABLE generation_snapshots DROP COLUMN prompt_version_id",
        "DROP INDEX IF EXISTS idx_generation_asset_versions_asset_version",
        "DROP INDEX IF EXISTS idx_generation_asset_versions_generation_created",
        "DROP TABLE IF EXISTS generation_asset_versions",
        "DROP INDEX IF EXISTS idx_generation_tool_usages_tool_instance_created",
        "DROP INDEX IF EXISTS idx_generation_tool_usages_generation_created",
        "DROP TABLE IF EXISTS generation_tool_usages",
        "DROP INDEX IF EXISTS idx_tool_instances_last_checked",
        "DROP INDEX IF EXISTS idx_tool_instances_tool_status",
        "DROP TABLE IF EXISTS tool_instances",
        "DROP INDEX IF EXISTS idx_tool_capabilities_name",
        "DROP TABLE IF EXISTS tool_capabilities",
        "DROP INDEX IF EXISTS idx_tool_versions_tool_observed",
        ...
        sqlx::query(statement)
            .execute(pool)
            .await
            .expect("034 model foundation should be removable in isolated upgrade fixture");
    }
    ...
    sqlx::query("DROP TABLE IF EXISTS workflow_recipe_runtime_states")
        .execute(pool)
        .await
        .expect("031 recipe archive table should be removable in isolated fixture");
    sqlx::query("DROP TABLE IF EXISTS workflow_recipe_promotions")
        .execute(pool)
        .await
        .expect("030 recipe promotion table should be removable in isolated fixture");
    sqlx::query("DROP INDEX IF EXISTS idx_recipes_workflow_version_id_id")
        .execute(pool)
        .await
        .expect("030 recipe promotion index should be removable in isolated fixture");
    sqlx::query("DROP TABLE IF EXISTS workflow_runtime_artifacts")
        .execute(pool)
        .await
        .expect("028 runtime artifact table should be removable in isolated fixture");
    for column in ["removed_at", "library_state", "source_kind"] {
        sqlx::query(&format!("ALTER TABLE workflows DROP COLUMN {column}"))
            .execute(pool)
            .await
            .expect("028 workflow registry columns should be removable in isolated fixture");
    }
    sqlx::query("DROP TABLE IF EXISTS project_workflow_bindings")
        .execute(pool)
        .await
        .expect("027 project workflow table should be removable in isolated upgrade fixture");
    sqlx::query("DROP TABLE IF EXISTS production_package_batch_bindings")
        .execute(pool)
        .await
        .expect("026 provenance table should be removable in isolated upgrade fixture");
...
#[tokio::test]
async fn dev048_fresh_migration_001_to_037_creates_only_the_frozen_tables() {
...
#[tokio::test]
async fn dev048_021_to_037_preserves_all_legacy_sentinels_and_leaves_new_tables_empty() {
...
#[tokio::test]
async fn dev052_existing_023_to_037_creates_preparation_snapshot_table() {
    let (directory, pool) = setup().await;
...
fn dev048_version_migration_and_scope_gate_is_explicit() {
    ...
    assert!(migrations.iter().all(|name| {
        name.get(..3)
            ...
            .is_some_and(|version| version <= 37)
    }));
