...
本 DEV 复用既有 `ProductionPackageWorkspace`、`ShotWorkspace` 和 `ProductionQueueDrawer`，不改变 Production Package V1 JSON schema 或后端生产链。Tauri V2 的拖放实现使用已安装 `@tauri-apps/api/webview` 的 `getCurrentWebview().onDragDropEvent()`；若运行环境无法注册，则状态为 `DRAG_DROP = API_UNAVAILABLE` 并继续使用选择器。最终验证结果随本次功能提交和 Source-only CI 一并记录。
