...
| P05-05 Runtime parameter profiles | Local, workflow-scoped profiles for steps, dimensions, duration, and queue concurrency; bounded and applied only to matching integer inputs | CODE PASS |
| P05-06 Workflow import quality | JSON size/shape checks, API-vs-UI format detection, credential/path warnings, node/output quality signals | CODE PASS |
